...
const initDataTable = async () => {
    ...
    await medico_detalle();

    dataTable = $("#datatable-medico").DataTable(dataTableOptions);
...
const medico_detalle = async () => {
    try {
        ...
        let content = ``;
        data.Medico.forEach((Medicos, list_medico) => {
            content += `
                <tr data-medico_id="${Medicos.id}">
                    <td>${list_medico + 1}</td>
                    <td>${Medicos.nombre}</td>
                    <td>${Medicos.apellido}</td>
                    <td>${Medicos.dpi}</td>
                    <td>${Medicos.numero_colegiado}</td>
                    <td>${Medicos.especialidad}</td>
                    <td>${Medicos.fecha_nacimiento}</td>
                    <td>${Medicos.telefono}</td>
                    <td>${Medicos.correo}</td>
                    <td>${Medicos.genero}</td>
        ...
        $(document).on('click', '.btn-eliminar', function() {
            var medicoId = $(this).closest('tr').data('medico_id');

            if (confirm('¿Estás seguro de que quieres eliminar este medico?')) {
